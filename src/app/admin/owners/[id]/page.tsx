...
function Fact({ label, value }: { label: string; value: string }) {
  return (
    <div className="min-w-0 rounded-xl bg-neutral-50 p-3">
      ...
      <dd className="mt-0.5 break-words text-sm font-medium text-neutral-900">
        {value}
      </dd>
    </div>
  );
    ...
    <div className="space-y-5">
      ...
      <section className="rounded-2xl border border-neutral-200 bg-white p-5">
        ...
        <dl className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-3">
          ...
          <Fact
            label="Plan"
            value={owner.plan ? owner.plan.toLowerCase() : "—"}
          />
      ...
      <section className="rounded-2xl border border-neutral-200 bg-white p-5">
        <div className="flex items-center justify-between gap-3">
          ...
          <Link
            href={`/admin/owners/${owner.id}/subscription-export`}
        ...
        {owner.subscriptionEvents.length === 0 ? (
          <p className="mt-2 text-sm text-neutral-500">
            No subscription or payment activity yet.
          </p>
        ) : (
          <ul className="mt-3 divide-y divide-neutral-100">
            {owner.subscriptionEvents.map((event) => (
              ...
              >
                ...
                <span className="text-neutral-500">
                  {event.status} · {dateLabel(event.createdAt)}
                </span>
              </li>
            ))}
          </ul>
