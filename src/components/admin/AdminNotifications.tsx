  ...
  return (
    <div className="relative">
      ...
      >
        ...
        {unread > 0 && (
          <span className="absolute right-1 top-1 min-w-4 rounded-full bg-rose-600 px-1 text-[10px] font-semibold text-white">
            {unread > 9 ? "9+" : unread}
          </span>
        )}
      </button>
      ...
      {open && (
        <section
          className="absolute right-0 z-50 mt-2 w-[min(22rem,calc(100vw-2rem))] overflow-hidden rounded-xl border border-neutral-200 bg-white shadow-lg"
            ...
            <ul className="max-h-96 divide-y divide-neutral-100 overflow-y-auto">
              {notifications.map((notification) => (
                <li
                  key={notification.id}
                  className={
                    notification.readAt ? "px-4 py-3" : "bg-sky-50 px-4 py-3"
                  }
                >
                  <button
                    type="button"
                    ...
                    onClick={() =>
                      startTransition(() => {
                        void markAdminNotificationReadAction(notification.id);
                      })
                    }
