...
export const roomSchema = z
  .object({
    label: z.preprocess(
      normalizeTextInput,
      z.string().trim().min(1, "Room name is required").max(60),
    ),
...
const contactNumberSchema = z
  .object({
    ...
    carrier: z
      .string()
      .trim()
      .max(CARRIER_MAX_LENGTH, "Carrier name is too long")
      .default(""),
  })
  .strict();
...
export const listingSchema = z
  .object({
    name: z.preprocess(
      normalizeTextInput,
      z.string().trim().min(3, "Name is too short").max(120),
    ),
    addressLine: z.preprocess(
      normalizeTextInput,
      z.string().trim().min(5, "Address is too short").max(160),
    ),
    ...
    houseRules: z
      .preprocess(
        normalizeTextInput,
        z.string().trim().max(1000, "Keep house rules under 1000 characters"),
      )
      .optional(),
