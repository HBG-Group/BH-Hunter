...
// Delete only if the review belongs to this student.
export async function deleteOwnReview(reviewId: string, studentId: string): Promise<boolean> {
  const result = await prisma.review.deleteMany({ where: { id: reviewId, authorId: studentId } });
