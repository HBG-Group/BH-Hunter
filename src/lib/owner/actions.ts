...
export async function createListingAction(
  ...
  if (result.data.rooms.length > entitlement.roomLimit) {
    return {
      error: `${entitlement.planName} allows up to ${entitlement.roomLimit} rooms per listing.`,
    };
  }
  ...
  if (quota.atLimit) {
    return {
      error: `You've used all ${quota.freeLimit} free listings. Contact the admin to add more (₱${quota.extraPrice} each).`,
    };
  }
  ...
  await invalidate(
    "listings:published",
    `listing:${created.slug}`,
    `metrics:owner:${owner.id}`,
  );
...
export async function updateListingAction(
  ...
  const beforeState = before
    ? summarizeAvailability(before.rooms).state
    : "FULL";
...
export async function requestVerificationAction(): Promise<{ ok: boolean }> {
  ...
  await notifyAdmins({
    ...
    title: "Owner requested verification",
    body: `${owner.fullName} submitted a verification request.`,
  });
  revalidatePath("/owner");
  return { ok: true };
}
...
export async function deleteViewingRequestAction(
  id: string,
): Promise<{ error?: string }> {
  const owner = await requireWritableOwner();
  if (!(await allow("write", LIMITS.write, owner.id)))
...
export async function setStatusAction(
  ...
  if (status === "PENDING") {
    ...
    if (photos < MIN_LISTING_PHOTOS)
      return { error: PHOTO_REQUIREMENT_MESSAGE };
  }
