...
| Community Guidelines | `/community-guidelines` | Rules + consequences for students and owners. |
...
There is no in-product report/flag feature (listing/review/owner/student reports were
built, then intentionally removed). Trust and safety issues are handled through
`docs/MODERATION.md`'s procedure, triggered by a direct message via `/contact` or the
admin's own review of listings/reviews.
...
advertisement create/status/delete.
...
(`verificationStatus`, the legal pages) follows the same pattern as the rest of the app:
`requireProfile`/`requireAdmin` gating, server-side Zod validation where relevant, and no
...
- If a report/flag feature is reintroduced later, reuse the `ModerationEvent` audit
  trail for its resolution actions rather than inventing a parallel log.
